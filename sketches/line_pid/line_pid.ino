...
const long interval = 30; // Interval for sensor reading and control update
...
void PIDControl() {
    int error = sensors[2] - line_thresh; // Center sensor as reference
    integral = integral + error;
    float derivative = error - previousError;
    float output = Kp * error + Ki * integral + Kd * derivative;

    setLeftMotorSpeed(255 - output);
    setRightMotorSpeed(255 + output);
